...
def get_arguments():
    ...
    parser.add_argument("--save-three-channels", type=str)
...
logits은 preprocessing이 필요한 값
padding 제거 -> max -> resize
'''
def preprocess_logits(logits):
    # part_am = logits[:,65:-100] #duke_query
    # part_am = logits[:,50:-50]  #market_query
    # part_am = logits[:,60:-90]  #duke_test
    part_am = logits[:,70:-70]  #duke_train
    part_am = np.max(part_am, axis=-1)
    resized_image = cv2.resize(part_am, (64, 128))
...
def main():
    ...
    #MIN, MAX 초깃값
    MIN_UPPER, MAX_UPPER = 999, -999
    MIN_BOTTOM, MAX_BOTTOM = 999, -999
    MIN_BKG, MAX_BKG = 999, -999
    ##############################################################################
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(testloader)):
            ...
            ##############################################################################
            #min 값 재설정
            if MIN_UPPER > np.min(_[:,:,[5, 7]]):
                MIN_UPPER = np.min(_[:,:,[5, 7]])
            if MIN_BOTTOM> np.min(_[:,:,[9, 12]]):
                MIN_BOTTOM= np.min(_[:,:,[9, 12]])
            if MIN_BKG > np.min(_[:,:,[0]]):
                MIN_BKG = np.min(_[:,:,[0]])

            #max 값 재설정
            if MAX_UPPER < np.max(_[:,:,[5, 7]]):
                MAX_UPPER = np.max(_[:,:,[5, 7]])
            if MAX_BOTTOM < np.max(_[:,:,[9, 12]]):
                MAX_BOTTOM= np.max(_[:,:,[9, 12]])
            if MAX_BKG < np.max(_[:,:,[0]]):
                MAX_BKG = np.max(_[:,:,[0]])
            ############################################################################
        
        '''
        minmax 정규화, 하나의 logit 마다 진행
        - Stack 하면서 모든 logits 을 저장하고 이를 load해 저장하는 것은 많은 메모리 및 시간이 소요된다.
        '''
        for idx, batch in enumerate(tqdm(testloader)):
            image, meta = batch
            if (len(image.shape) > 4):
            ...
            #Step1. minmax 정규화
            MINMAX_UPPER_ONE_LOGIT = (deepcopy(_[:,:,[5,7]]) - MIN_UPPER) / (MAX_UPPER - MIN_UPPER)*255         #N*H*W*CHANNEL
            MINMAX_BOTTOM_ONE_LOGIT = (deepcopy(_[:,:,[9, 12]]) - MIN_BOTTOM) / (MAX_BOTTOM - MIN_BOTTOM)*255    #N*H*W*CHANNEL
            MINMAX_BKG_ONE_LOGIT = (deepcopy(_[:,:,[0]]) - MIN_BKG) / (MAX_BKG - MIN_BKG)*255    #N*H*W*CHANNEL

            PRE_NORM_UPPER_ONE_LOGIT, PRE_NORM_BOTTOM_ONE_LOGIT, PRE_NORM_BKG_ONE_LOGIT = preprocess_logits(MINMAX_UPPER_ONE_LOGIT), preprocess_logits(MINMAX_BOTTOM_ONE_LOGIT), preprocess_logits(MINMAX_BKG_ONE_LOGIT)
            
            output_im1 = convert_to_l(PRE_NORM_UPPER_ONE_LOGIT)
            output_im2 = convert_to_l(PRE_NORM_BOTTOM_ONE_LOGIT)
            output_im3 = convert_to_l( PRE_NORM_BKG_ONE_LOGIT)

            #Step2. upper, bottom, bkg 에 해당하는 activation map stack
            stacked_image = np.stack((output_im1, output_im2, output_im3), axis=-1)
            plt.imshow(stacked_image)

            SAVE_THREE_CHANNELS = args.save_three_channels

            stacked_image = Image.fromarray(np.asarray(stacked_image, dtype=np.uint8))
            stacked_image.save(os.path.join(SAVE_THREE_CHANNELS, im_name+'.png'))
